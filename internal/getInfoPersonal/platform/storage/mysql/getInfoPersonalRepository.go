...
func (g *GetInfoPersonalRepository) GetInfoPersonalRepo(ctx context.Context) ([]getInfoPersonal.GetInfoPersonalResponse, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&respDB.Id, &respDB.FirstName, &respDB.SecondName, &respDB.LastName, &respDB.SecondLastName, &respDB.Sex, &respDB.DateOfBirth, &respDB.TypeDocument, &respDB.DocumentNumber, &respDB.User, &respDB.TypeUser, &respDB.DateCreationAccount, &respDB.StateName); err != nil {
